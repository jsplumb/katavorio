...
// --------------------------- DRAG SELECTION -----------------------------------------------
    ...
    test("multiple delegate selectors on one Drag object, destroy cleans them up ", function () {
        var d = _add("d1", null, [0,0], [500,500]),
            foo = _add("foo", d, [20,20], [50,50]),
            bar = _add("bar", d, [100,100], [50,50]),
            m = new Mottle(),
        ...
        k.destroyDraggable(d);

        stopped = false;
        draggedChild = null;
        // move 'foo' around
        _t(m, foo, "mousedown", 30, 30);
        _t(m, document, "mousemove", 200, 200);
        ok(!foo.classList.contains("katavorio-drag"), "drag class NOT set on element");
        m.trigger(document, "mouseup");
        ok(!stopped, "stop event was NOT fired");
        equal(parseInt(foo.style.left, 10), 90, "left position of foo correct after drag didnt happen");
        equal(parseInt(foo.style.top, 10), 90, "top position of foo correct after drag didnt happen");
